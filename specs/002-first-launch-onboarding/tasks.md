...
- [x] T003 [US1] Maintain ordered notification -> language -> location -> Login state flow in `lib/features/onboarding/ui/view_models/first_launch_view_model.dart`.
...
- [x] T010 [US1] Report onboarding answers best-effort via `lib/features/onboarding/data/services/device_launch_service.dart` and `POST /devices/launch` in `prototype_server/lib/routes/partner_routes.dart`.
...
- [x] T007 [US2] Verify denied and skipped paths in `test/features/onboarding/first_launch_view_model_test.dart`.
- [ ] T011 [US2] Add a test for the Open Settings path in `test/features/onboarding/first_launch_view_model_test.dart`.
